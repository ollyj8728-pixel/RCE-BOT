...
client.on('interactionCreate', async i => {
  try {
    ...
    if (i.isStringSelectMenu() && i.customId === 'setup_owner_role') { const roleId=i.values[0]; if(roleId==='none') return i.reply({content:'No role selected. Use Auto-Create.',ephemeral:true}); db.prepare('INSERT OR IGNORE INTO setup_progress (guild_id) VALUES (?)').run(i.guildId); db.prepare('UPDATE setup_progress SET roles_done=1 WHERE guild_id=?').run(i.guildId); return i.update(setupPanel(i.guildId)); }
    if (i.isButton() && i.customId === 'setup_owner_auto') { const role=await i.guild.roles.create({name:'Valora Owner',color:0x2563eb,reason:'Valora RCE setup Owner Role'}); db.prepare('INSERT OR IGNORE INTO setup_progress (guild_id) VALUES (?)').run(i.guildId); db.prepare('UPDATE setup_progress SET roles_done=1 WHERE guild_id=?').run(i.guildId); return i.update(setupPanel(i.guildId)); }
    if (i.isButton() && i.customId === 'setup_reset') { db.prepare('INSERT OR REPLACE INTO setup_progress (guild_id,roles_done,logs_done,server_done) VALUES (?,0,0,0)').run(i.guildId); return i.update(setupPanel(i.guildId)); }
