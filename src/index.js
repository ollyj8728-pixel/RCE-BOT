...
client.once('ready', async () => {
  ...
  try {
    ...
    log.info({user:client.user.tag,brand:BRAND,commandCount:commands.length}, 'Rustworks RCE bot online');
  } catch (error) {
...
client.on('interactionCreate', async i => {
  try {
    ...
    if (i.isButton() && i.customId === 'nav_help') return i.reply({content:'Use `/help` any time to reopen the Rustworks RCE command guide.',ephemeral:true});
    ...
    if (i.isButton() && i.customId === 'setup_owner_auto') { const role=await i.guild.roles.create({name:'Rustworks Owner',color:0x2563eb,reason:'Rustworks setup Owner Role'}); db.prepare('INSERT OR IGNORE INTO setup_progress (guild_id) VALUES (?)').run(i.guildId); db.prepare('UPDATE setup_progress SET roles_done=1 WHERE guild_id=?').run(i.guildId); return i.update(setupPanel(i.guildId)); }
    ...
    if (i.isModalSubmit() && i.customId.startsWith('setup_modal:')) { const step=i.customId.split(':')[1], value=i.fields.getTextInputValue('value'); if(step==='roles'){await i.guild.roles.create({name:value,color:0x2563eb,reason:'Rustworks setup'});} db.prepare('INSERT OR IGNORE INTO setup_progress (guild_id) VALUES (?)').run(i.guildId); db.prepare(`UPDATE setup_progress SET ${step}_done=1 WHERE guild_id=?`).run(i.guildId); return i.reply({content:`✅ Step complete: ${step}. ${step==='roles'?'The role was created.':`Saved: ${value}`}`,ephemeral:true}); }
